...
headers1 = ['Tháng/Năm', 'Công ty', 'Mảng kinh doanh', 'Đầu kỳ', '', 'Tăng trong kỳ', '', 'Giảm trong kỳ', '', 'Cuối kỳ', '']
...
ws_cust.merge_cells('D2:E2')
ws_cust.merge_cells('F2:G2')
ws_cust.merge_cells('H2:I2')
ws_cust.merge_cells('J2:K2')
...
for comp in companies:
    ...
    for cat in categories:
        ...
        if cat == 'Phân phối (Đại lý)':
            ws_cust.cell(row=row, column=4, value='-')
            ws_cust.cell(row=row, column=6, value='-')
            ws_cust.cell(row=row, column=8, value='-')
            ws_cust.cell(row=row, column=10, value='-')
        row += 1
...
artifact_path = r'C:\Users\Hp\.gemini\antigravity\brain\c2c07afe-a201-42e1-b52e-07538b848a35\Template_Nhap_Lieu_VPS_V3_LuyKe.xlsx'
